...
export const SYNC_ENTITIES: SyncEntityDefinition[] = [
  // ─── Tenant / Users (ordre 0-10) ───
  {
    ...
    remoteTable: 'tenants',
    syncOrder: 0,
    allowedFields: [...ALLOWED_BASE, 'slug', 'name', 'legal_name', 'owner_email', 'owner_phone',
      'contact_email', 'contact_phone', 'country', 'city', 'address', 'logo_url', 'primary_color',
      'default_currency', 'default_locale', 'timezone', 'status', 'is_provisioned'],
    hasUpdatedAt: true,
    hasTenantId: false,
  },
  {
  ...
  // ─── Catalogue (ordre 10-20) ───
  {
    ...
    syncOrder: 10,
    allowedFields: [...ALLOWED_BASE, 'name', 'description'],
    hasUpdatedAt: true,
    hasTenantId: true,
  },
  {
  ...
  // ─── Tables (ordre 20) ───
  {
    ...
    syncOrder: 20,
    allowedFields: [...ALLOWED_BASE, 'table_number', 'capacity', 'status', 'assigned_waiter_id', 'qr_token'],
