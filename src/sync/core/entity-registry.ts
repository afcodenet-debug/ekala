...
export const SYNC_ENTITIES: SyncEntityDefinition[] = [
  // ─── Tenant / Users (ordre 0-10) ───
  {
    ...
    allowedFields: ['created_at', 'updated_at', 'remote_id', 'slug', 'name', 'legal_name', 'owner_email', 'owner_phone',
      ...
      'tenant_id', 'suspended_at', 'suspension_reason', 'suspended_by', 'last_reactivated_at', 'last_reactivated_by', 'disabled_at', 'disabled_by'],
  ...
  },
  {
    ...
    remoteTable: 'users',
    syncOrder: 5,
    allowedFields: [...ALLOWED_BASE, 'full_name', 'username', 'pin_code', 'role', 'is_active', 'email', 'tenant_id', 'phone', 'password_hash', 'has_setup_pin', 'is_super_admin', 'is_platform_user', 'status', 'revoked_at', 'revoked_by', 'locked_until'],
    // is_platform_user is stored as INTEGER (0/1) locally but BOOLEAN in Supabase
    booleanFields: ['is_active', 'has_setup_pin', 'is_platform_user'],
    // revoked_by is a self-reference to users (resolve to remote_id)
