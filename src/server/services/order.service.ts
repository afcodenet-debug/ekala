...
export class OrderService {
  private static getItemsForOrder(orderId: number, fallbackJson?: string, isRemote: boolean = false): OrderItem[] {
    ...
    try {
      if (dataSource.isCloudMode()) {
        if (fallbackJson) {
          try { return JSON.parse(fallbackJson || '[]') as OrderItem[]; } catch { return []; }
        }
        return [];
      }
      ...
      if (isRemote && fallbackJson) {
        ...
        try {
          ...
          if (typeof data === 'string') data = JSON.parse(data);
        } catch (e: any) { console.warn('[OrderService] fallbackJson parse error:', e.message); }

        let itemsArray: any[] = [];
        if (Array.isArray(data)) itemsArray = data;
        else if (data && typeof data === 'object' && Array.isArray(data.items)) itemsArray = data.items;
        else if (data && typeof data === 'object') { if (data.product_id || data.productId) itemsArray = [data]; }
        ...
        return itemsArray.map((it: any) => {
          ...
          return { id: it.id, productId: pid, name: it.name || prod?.name || 'Unknown', price, quantity: Number(it.quantity) || 0, notes: it.notes ?? undefined };
        }).filter(Boolean) as OrderItem[];
      }

      const items = db.prepare(`
        SELECT oi.id, oi.product_id AS productId, p.name, oi.unit_price AS price, oi.quantity, oi.notes
        FROM order_items oi LEFT JOIN products p ON oi.product_id = p.id
        WHERE oi.order_id = ? AND oi.tenant_id = ?
      `).all(orderId, tenantId) as any[];

      if (items.length > 0) return items.map(item => ({ id: item.id, productId: item.productId, name: item.name || '', price: item.price, quantity: item.quantity, notes: item.notes ?? undefined }));

      if (fallbackJson) { try { return JSON.parse(fallbackJson || '[]') as OrderItem[]; } catch { return []; } }
  ...
  private static resolveEffectiveWaiterId(table_id: number | null, waiter_id: number): number {
    ...
    const table = db.prepare('SELECT assigned_waiter_id FROM restaurant_tables WHERE id = ? AND tenant_id = ?').get(table_id, tenantId) as { assigned_waiter_id: number | null } | undefined;
  ...
  /**
   * Insère les items d'une commande et décrémente le stock.
   * FIX: Le stock est maintenant déduit à la création de la commande,
   * pas seulement au checkout. Évite les divergences entre stock local et cloud.
   */
  private static insertOrderItems(orderId: number, items: OrderItem[]): void {
    ...
    for (const item of items as any[]) {
      ...
      if (sync) {
        ...
        if (product) {
          sync.queueChangeInsideTransaction(
            'product',
            'update',
            {
              id: product.id,
  ...
  static async getAll(params: { waiter_id?: number; role?: string; table_id?: number; status?: string } = {}): Promise<OrderData[]> {
    ...
    if (dataSource.isCloudMode()) {
      ...
      try {
        ...
        if (role === 'waiter' && waiter_id) query = query.eq('waiter_id', waiter_id);
        ...
        return (data || []).map((order: any) => ({
          ...
          table_waiter_name: order.table?.waiter?.full_name || order.table?.waiter?.username,
          order_waiter_name: order.waiter?.full_name || order.waiter?.username,
          waiter_name: (order.table?.waiter?.full_name || order.table?.waiter?.username) || (order.waiter?.full_name || order.waiter?.username),
          waiter_role: order.table?.waiter?.role || order.waiter?.role,
          items: typeof order.items === 'string' ? JSON.parse(order.items) : (order.items || [])
        }));
      } catch (err: any) { console.error('[OrderService] Supabase getAll failed:', err?.message || err); throw new Error('Failed to fetch orders via Supabase'); }
    }

    let query = `
      SELECT o.*, COALESCE(t.table_number, t2.table_number) as table_number,
        ...
        TRIM(COALESCE(u.full_name, '') || ' ' || COALESCE(u.username, '')) as order_waiter_name, ut.role as waiter_role
      FROM orders o
    ...
    if (role === 'waiter' && waiter_id) { query += ` AND (o.waiter_id = ? OR o.waiter_id = (SELECT remote_id FROM users WHERE id = ?))`; queryParams.push(waiter_id, waiter_id); }
    if (table_id) { query += ` AND o.table_id = ?`; queryParams.push(table_id); }
    ...
    try {
      ...
      for (const order of orders) {
        if (order.table_id && !order.table_number) {
          let matchByRemote = db.prepare(`SELECT id, table_number FROM restaurant_tables WHERE remote_id = ? AND tenant_id = ?`).get(order.table_id, tenantId) as any;
          if (!matchByRemote && typeof order.table_id === 'string') matchByRemote = db.prepare(`SELECT id, table_number FROM restaurant_tables WHERE table_number = ? AND tenant_id = ?`).get(order.table_id, tenantId) as any;
          if (!matchByRemote) matchByRemote = db.prepare(`SELECT id, table_number FROM restaurant_tables WHERE id = ? AND tenant_id = ?`).get(order.table_id, tenantId) as any;
          if (matchByRemote) { order.table_id = matchByRemote.id; order.table_number = matchByRemote.table_number; }
        }
      }
      return orders.map(order => ({ ...order, items: this.getItemsForOrder(order.id, order.items, !!order.remote_id) }));
    } catch (error: any) { console.error('[OrderService] Error fetching orders:', error); throw new Error('Failed to fetch orders: ' + (error.message || 'unknown')); }
  }
  ...
  static async getById(id: number): Promise<OrderData | null> {
    ...
    if (dataSource.isCloudMode()) {
      try {
        ...
        if (productIds.length > 0) {
          const { data: products } = await supabase.from('products').select('id, name, selling_price').in('id', productIds).eq('tenant_id', tenantId);
          if (products) products.forEach((p: any) => productsMap.set(p.id, p));
        }
        const enrichedItems = rawItems.map((it: any) => {
          const pid = it.product_id || it.productId;
          const product = productsMap.get(pid);
          return { id: it.id, productId: pid, name: it.name || product?.name || 'Unknown Product', price: Number(it.price || it.unit_price || product?.selling_price || 0), quantity: Number(it.quantity) || 0, notes: it.notes ?? undefined };
        });
        ...
        return { ...data, table_number: data.table?.table_number, table_waiter_name: tableWaiterName, order_waiter_name: orderWaiterName, waiter_name: tableWaiterName || orderWaiterName, items: enrichedItems };
      } catch (err: any) { console.error('[OrderService] Supabase getById failed:', err?.message || err); throw new Error('Failed to fetch order via Supabase'); }
    }
    ...
    try {
      ...
      if (order) return { ...order, items: this.getItemsForOrder(order.id, order.items, !!order.remote_id) };
      return null;
    } catch (error) { console.error('[OrderService] Error fetching order:', error); throw new Error('Failed to fetch order'); }
  }
  ...
  static async create(orderData: Omit<OrderData, 'id' | 'created_at' | 'updated_at'>): Promise<OrderData> {
    ...
    if (dataSource.isCloudMode()) {
      try {
        ...
        const normalizedItems = items.map((item: any) => ({ ...item, productId: item.productId ?? item.product_id, name: item.name || '', quantity: Number(item.quantity), price: Number(item.price), notes: item.notes ?? null, tenant_id: tenantId }));
        ...
        try {
          ...
          console.error(`[Order] Failed to trigger product sync:`, syncErr?.message || syncErr);
        }

        return { ...data, items: normalizedItems };
      } catch (err: any) { console.error('[OrderService] Supabase create failed:', err?.message || err); throw new Error(err?.message || 'Failed to create order via Supabase'); }
    }
    ...
    return withOutboxTransaction(db, String(tenantId), () => {
      try {
        ...
        if (table_id) {
          const existingOrder = db.prepare(`SELECT id, items, version FROM orders WHERE table_id = ? AND tenant_id = ? AND status NOT IN ('paid', 'cancelled') ORDER BY created_at DESC LIMIT 1`).get(table_id, tenantId) as any;
          if (existingOrder) {
            ...
            normalizedItems.forEach((newItem: OrderItem) => {
              ...
              else itemMap.set(newItem.productId, { ...newItem });
            });
            const mergedItems = Array.from(itemMap.values());
            const mergedTotal = mergedItems.reduce((sum, item) => sum + item.price * item.quantity, 0);
            db.prepare(`UPDATE orders SET items = ?, total = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND tenant_id = ?`).run(JSON.stringify(mergedItems), mergedTotal, existingOrder.id, tenantId);
        ...
        const result = db.prepare(`INSERT INTO orders (table_id, waiter_id, items, status, total, tenant_id, source, created_at, updated_at, version) VALUES (?, ?, ?, ?, ?, ?, 'local', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 1)`).run(table_id, effectiveWaiterId, JSON.stringify(normalizedItems), status || 'pending', total, tenantId);
  ...
  static async updateItems(id: number, items: OrderItem[]): Promise<OrderData> {
    ...
    if (dataSource.isCloudMode()) {
      try {
        const { getSupabaseClient } = require('../database/supabase.client');
        const supabase = getSupabaseClient();
        const normalizedItems = items.map(item => ({ ...item, name: item.name || '', quantity: Number(item.quantity), price: Number(item.price), notes: item.notes ?? null, tenant_id: tenantId }));
        const total = normalizedItems.reduce((sum, item) => sum + item.price * item.quantity, 0);
        const { data, error } = await supabase.from('orders').update({ items: normalizedItems, total, updated_at: new Date().toISOString() }).eq('id', id).eq('tenant_id', tenantId).select().single();
        if (error) throw error;
        if (!data) throw new Error('Order not found');
        try { mirrorRemoteOrderToLocal(tenantId, data, normalizedItems); } catch (mirrorErr: any) { console.warn('[OrderService] Cloud→SQLite mirror failed:', mirrorErr?.message); }
        return { ...data, items: normalizedItems };
      } catch (err: any) { console.error('[OrderService] Supabase updateItems failed:', err?.message || err); throw new Error(err?.message || 'Failed to update order items via Supabase'); }
    }
    ...
    return withOutboxTransaction(db, String(tenantId), () => {
      try {
        ...
        const normalizedItems = items.map(item => ({ ...item, name: item.name || '', quantity: Number(item.quantity), price: Number(item.price), notes: item.notes ?? null, tenant_id: tenantId }));
        const total = normalizedItems.reduce((sum, item) => sum + item.price * item.quantity, 0);
        db.prepare(`UPDATE orders SET items = ?, total = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND tenant_id = ?`).run(JSON.stringify(normalizedItems), total, id, tenantId);
  ...
  static async updateStatus(id: number, status: OrderData['status']): Promise<OrderData> {
    ...
    if (dataSource.isCloudMode()) {
      try {
        const { getSupabaseClient } = require('../database/supabase.client');
        const supabase = getSupabaseClient();
        const { data, error } = await supabase.from('orders').update({ status, updated_at: new Date().toISOString() }).eq('id', id).eq('tenant_id', tenantId).select('*, table:restaurant_tables(table_number, assigned_waiter_id, waiter:users(full_name, username)), waiter:users(full_name, username)').single();
        if (error) throw error;
        if (!data) throw new Error('Order not found');
        try { mirrorRemoteOrderToLocal(tenantId, data); } catch (mirrorErr: any) { console.warn('[OrderService] Cloud→SQLite mirror failed:', mirrorErr?.message); }
        if (status === 'paid' && data.table_id) { await supabase.from('restaurant_tables').update({ status: 'cleaning' }).eq('id', data.table_id).eq('tenant_id', tenantId); }
        else if (status === 'cancelled' && data.table_id) {
          const { count } = await supabase.from('orders').select('*', { count: 'exact', head: true }).eq('table_id', data.table_id).eq('tenant_id', tenantId).not('status', 'in', '("paid","cancelled")');
          if (count === 0) await supabase.from('restaurant_tables').update({ status: 'available' }).eq('id', data.table_id).eq('tenant_id', tenantId);
        }
        const tableWaiterName = data.table?.waiter?.full_name || data.table?.waiter?.username;
        const orderWaiterName = data.waiter?.full_name || data.waiter?.username;
        return { ...data, table_number: data.table?.table_number, table_waiter_name: tableWaiterName, order_waiter_name: orderWaiterName, waiter_name: tableWaiterName || orderWaiterName, items: typeof data.items === 'string' ? JSON.parse(data.items) : (data.items || []) };
      } catch (err: any) { console.error('[OrderService] Supabase updateStatus failed:', err?.message || err); throw new Error(err?.message || 'Failed to update order status via Supabase'); }
    }
    ...
    return withOutboxTransaction(db, String(tenantId), () => {
      try {
        ...
        if (status === 'paid') {
          ...
          if (order && order.table_id) db.prepare("UPDATE restaurant_tables SET status = 'cleaning' WHERE id = ? AND tenant_id = ?").run(order.table_id, tenantId);
        } else if (status === 'cancelled') {
          const order = db.prepare('SELECT table_id FROM orders WHERE id = ? AND tenant_id = ?').get(id, tenantId) as any;
          if (order && order.table_id) {
            const activeOrders = db.prepare(`SELECT COUNT(*) as count FROM orders WHERE table_id = ? AND tenant_id = ? AND status NOT IN ('paid', 'cancelled')`).get(order.table_id, tenantId) as any;
            if (activeOrders.count === 0) db.prepare("UPDATE restaurant_tables SET status = 'available' WHERE id = ? AND tenant_id = ?").run(order.table_id, tenantId);
          }
        }
          ...
          if (!saleExists) {
            setImmediate(async () => {
              try {
                ...
                const orderItems = result.items.map((item: any) => ({ name: item.name, qty: item.quantity, unitPrice: item.price, total: item.price * item.quantity }));
                const tableLabel = result.table_id ? `Table ${result.table_number || result.table_id}` : 'Counter';
                await notifyOrderCheckout(id, orderItems, result.total, 'cash', tableLabel, result.waiter_name, undefined, 'USD', rawSettings);
              } catch (err) { console.error('[OrderService] Failed to send checkout notification:', err); }
            });
          }
        }
        return result;
  ...
  static validateOrderOperation(order: OrderData, operation: string): void {
    ...
    if (order.status === 'cancelled') throw new Error('Cannot operate on cancelled order');
  }
  ...
  static async deleteOrder(id: number): Promise<void> {
    ...
    if (dataSource.isCloudMode()) {
      try {
        const { getSupabaseClient } = require('../database/supabase.client');
        const supabase = getSupabaseClient();
        const { error } = await supabase.from('orders').delete().eq('id', id).eq('tenant_id', tenantId);
        if (error) throw error;
        try { deleteMirroredOrder(tenantId, id); } catch (mirrorErr: any) { console.warn('[OrderService] Cloud→SQLite mirror delete failed:', mirrorErr?.message); }
      } catch (err: any) { console.error('[OrderService] Supabase deleteOrder failed:', err?.message || err); throw new Error(err?.message || 'Failed to delete order via Supabase'); }
      return;
    }
